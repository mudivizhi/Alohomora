...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        firebaseDatabase = FirebaseDatabase.getInstance().getReference().child("user-profile");
    ...
    private void onLoggedIn(GoogleSignInAccount googleSignInAccount) {
        ...
        createUserInDB(firebaseDatabase,"email",googleSignInAccount.getEmail());
        finish();
    }

    public void createUserInDB(DatabaseReference db,String key,String value){
        db.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()) {
                    for (DataSnapshot userSnapshot : dataSnapshot.getChildren()) {
                        for(DataSnapshot specificUser : userSnapshot.getChildren()) {
                            if (key.equals(specificUser.getKey()) && value.equals(specificUser.getValue().toString())) {
                                return;
                            }
                        }
                    }
                }
                db.push().child(key).setValue(value);
            }
